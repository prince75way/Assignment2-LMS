...
export const createCourseService = async (courseData: CourseDTO) => {
  console.log(courseData)
    ...
    const decoded: any = jwt.verify(courseData.accessToken || "", process.env.ACCESS_TOKEN_SECRET);
    const userId = decoded.userId;
    console.log("userid is: ",userId)

  const newCourse = new Course({title:courseData.title,description:courseData.description,price:courseData.price,image:courseData.image,category:courseData.category,instructor:userId});
...
export const enrollInCourseService = async (accessToken: string, courseId: string): Promise<CourseDTO> => {
  ...
  const decodedToken=await validateToken(accessToken);
