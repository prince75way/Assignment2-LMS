 ...
 */
export const refreshAccessToken = async (accessToken: string): Promise<string> => {
...
export const getUserEnrollmentStatus = async (userId: string, courseId: string): Promise<boolean> => {
  // Validate the courseId first
  ...
  const course = await Course.findOne({ "_id": objectId }) as CourseDTO | null;
